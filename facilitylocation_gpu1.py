...
class SetFunction(nn.Module):
    ...
    def get_effective_ground_set(self, n) -> Set[int]:
...
# def create_square_kernel_dense(X_ground, metric: str = "euclidean"):
#     #print("create_square_kernel_dense")
...
#     #print(n_ground)
#     # if metric == "euclidean":
#     #     print("inside euclidean")
#     #     for r in range(0, n_ground, batch_size):
#     #             print(r)
#     #             X1_batch = X_ground[r:r+batch_size]
#     #             for c in range(0, n_ground, batch_size):
#     #                 X2_batch = X_ground[c:c+batch_size]
#     #                 sim_batch = euclidean_similarity(X1_batch, X2_batch)
#     #                 for i in range(len(X1_batch)):
#     #                     for j in range(len(X2_batch)):
#     #                         k_dense[r+i][c+j] = sim_batch[i][j].item()
#     #                         k_dense[c+j][r+i] = sim_batch[i][j].item()
...
#     #             k_dense[r][c] = sim
#     #             k_dense[c][r] = sim
#     #         print("k_dense",k_dense[1][2])
#     #     print("euclidean done")
#     #------
#     n_ground = len(X_ground)

#     if metric == "euclidean":
#         k_dense = euclidean_similarity(X_ground, X_ground)
#         k_dense.fill_diagonal_(1.0)
#         print("euclidean done")
#         print(k_dense[1][2])
...
#             k_dense[r][r] = 1.0
#             for c in range(r + 1, n_ground):
#                 sim = cosine_similarity(X_ground[r],X_ground[c])
...
#         distances = torch.cdist(X_ground, X_ground, p=2)
#         k_dense = torch.exp(-distances.pow(2))
#     elif metric == "cosine":
#         # Cosine similarity
#         X_ground_norm = torch.norm(X_ground, dim=1, keepdim=True)
#         dot_products = torch.mm(X_ground, X_ground.t())
#         k_dense = dot_products / torch.mm(X_ground_norm, X_ground_norm.t())
#     elif metric == "dot":
#         # Dot product
#         k_dense = torch.mm(X_ground, X_ground.t())
#     else:
...
def create_square_kernel_dense(X_ground, metric: str = "euclidean", batch_size: int = 100):
    n_ground = len(X_ground)
    k_dense = torch.zeros(n_ground, n_ground)
...
class FacilityLocationFunction(SetFunction):
                ...
                if self.mode == "clustered":
                    ...
                    if self.separate_rep == True:
                        ...
                        if self.mode == "dense":
                            ...
                            self.cpp_content = np.array(create_kernel(X = self.data.tolist(), metric = self.metric, mode = self.mode, num_neigh = self.num_neighbors).to_dense())
    ...
    def marginal_gain_with_memoization(self, X, item, enable_checks: bool = True):
        effective_X = torch.tensor([])  # Initialize as empty tensor
        gain = torch.tensor(0.0)  # Initialize gain as tensor

        if self.partial:
            effective_X = torch.tensor(list(X.intersection(self.effective_ground_set)), dtype=torch.long)
        else:
            effective_X = torch.tensor(list(X), dtype=torch.long)

        if enable_checks and item in effective_X:
            return torch.tensor(0.0)  # Return 0 as tensor
        ...
        if self.mode == 'dense':
            for ind in self.master_set:
                if self.partial:
                    max_sim = self.dense_kernel[ind, item]
                    if max_sim > self.similarity_with_nearest_in_effective_x[self.original_to_partial_index_map[ind]]:
                        gain += max_sim - self.similarity_with_nearest_in_effective_x[self.original_to_partial_index_map[ind]]
                else:
                    max_sim = self.dense_kernel[ind, item]
                    if max_sim > self.similarity_with_nearest_in_effective_x[ind]:
                        gain += max_sim - self.similarity_with_nearest_in_effective_x[ind]
        elif self.mode == 'sparse':
            for ind in self.master_set:
                max_sim = self.sparse_kernel[ind, item]
                if max_sim > self.similarity_with_nearest_in_effective_x[ind]:
                    gain += max_sim - self.similarity_with_nearest_in_effective_x[ind]
        else:  # clustered
            ...
            if len(relevant_subset) == 0:
                ...
                for ind in ci:
                    ind_ = self.cluster_index_map[ind]
                    max_sim = self.cluster_kernels[i][ind_][item_]
                    if max_sim > self.clustered_similarity_with_nearest_in_relevant_x[ind]:
                        gain += max_sim - self.clustered_similarity_with_nearest_in_relevant_x[ind]
    ...
    # def marginal_gain_with_memoization(self, X, item, enable_checks: bool = True):
    #     gain = torch.zeros(1, device="cuda")

    #     if enable_checks and item in X.intersection(self.effective_ground_set):
    #         return 0
    ...
    #     if self.partial:
    #         indices = self.original_to_partial_index_map
    #     else:
    #         indices = range(len(self.dense_kernel))  # Use all indices

    #     dense_items = torch.tensor([self.dense_kernel[ind][item] for ind in indices], device="cuda")
    #     similarity_items = torch.tensor(self.similarity_with_nearest_in_effective_x[indices], device="cuda")

    #     # Perform the operation on GPU directly
    #     #print(type(similarity_items))
    #     gains = torch.max(torch.zeros_like(dense_items), dense_items - similarity_items)
    #     return torch.sum(gains)  # Move the result to CPU and convert to NumPy array

    # def marginal_gain_with_memoization(self, X, items, enable_checks: bool = True):
    #     # Initialize a tensor to store the gains
    #     gains = torch.zeros(len(items), device="cuda")
    ...
    #         items = [item for item in items if item not in effective_X]
    #         if self.partial:
    #             items = [item for item in items if item in self.effective_ground_set]

    #     if self.partial:
    #         indices = [self.original_to_partial_index_map[item] for item in items]
    #     else:
    #         indices = list(range(len(self.dense_kernel)))  # Use all indices

    #     dense_items = torch.tensor([[self.dense_kernel[ind][item] for ind in indices] for item in items], device="cuda")
    #     similarity_items = torch.tensor(self.similarity_with_nearest_in_effective_x[indices], device="cuda")

    #     # Compute the gains using tensor operations
    #     gains = torch.max(torch.zeros_like(dense_items), dense_items - similarity_items)
    #     #print(torch.sum(gains, dim=1))
    #     return torch.sum(gains, dim=1)  # Sum along the columns to get marginal gains for each item in the set



    def marginal_gain_with_memoization_vector(self, X, R, enable_checks: bool = True):
        effective_X = set()
        ...
        gains = torch.zeros(len(R)).to(device="cuda")
        # R = R.to(device="cuda")

        if self.mode == 'dense':
            for ind in self.master_set:
                # ind = torch.tensor(ind).to(device="cuda")
                if self.partial:
                    ...
                    gains += torch.tensor(self.dense_kernel[ind][R]).to(device="cuda") - torch.tensor(self.similarity_with_nearest_in_effective_x[ind]).to(device="cuda")
    ...
    #     if self.mode == 'dense':
    #         # Check if the item has a higher similarity with any element in the master set
    #         item_similarities = torch.tensor([self.dense_kernel[ind][item] for ind in self.master_set], device="cuda")
    #         mask = item_similarities > self.similarity_with_nearest_in_effective_x
    #         self.similarity_with_nearest_in_effective_x[mask] = item_similarities[mask]
